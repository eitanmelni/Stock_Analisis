...
def analisis_macd(stocks):
    ...
    estado_rsi = ''
    for stock in stocks:
        dataframe = read_csv(f'Stocks Data\\{acc.tickers[stock]}.csv', index_col='Fecha', parse_dates=True)
        df2 = dataframe.loc[datetime.today() - timedelta(days=30):, ['Histogram']]
        ...
        df3 = dataframe.loc[datetime.today() - timedelta(days=15):, ['Histogram']]
        ...
        df4 = dataframe.loc[datetime.today() - timedelta(days=3):, ['Histogram']]
        ...
        if 1 <= volat <= 4 and tarde == 1 and cambio == 1 and dataframe.iloc[-1, 8] > 0:
            ...
            data.append([acc.tickers[stock], dataframe.iloc[-1, 5], dataframe.iloc[-1, 8], dataframe.iloc[-1, 9],
                         estado_rsi, dataframe.iloc[-1, 10]])
    oportunidades = DataFrame(data, acciones, cols)
